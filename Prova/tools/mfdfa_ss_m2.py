...
def getMSSByUpscaling(dx, normType=np.inf, isDFA=1, isNormalised=1):
    ...
    nScales = np.int(np.round(np.log2(
    ...
    for ji in range(1, nScales + 1):
        ...
        for k in range(1, n + 1):
            ...
            dx_k_withShifts = dx[(
                                             k - 1) * dx_k_len + 1 + dx_shift - dx_leftShift - 1: k * dx_k_len + dx_shift + dx_rightShift]
    ...
    stats = {'tau': tau, 'LH': LH, 'f': f, 'LH_min': LH_min, 'LH_max': LH_max, 'a_alfa': a_alfa, 'psi': psi,
             'delta_alfa': delta_alfa, 'alfa_0': alfa_0, 'h_min': h_min, 'h_max': h_max}
